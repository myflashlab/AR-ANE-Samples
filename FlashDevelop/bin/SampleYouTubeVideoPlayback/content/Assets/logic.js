...
function onReady()
{
	...
	arel.Events.setListener(arel.Media, function(type, mediaURL){onMedia(type, mediaURL);});
...
function onMedia(type, mediaURL)
{
	switch (type)
	{
		case arel.Events.Media.VIDEO_CLOSED:
...
function handleFullscreenEvents(obj, type, param)
{
	if (type && type === arel.Events.Object.ONTOUCHENDED)
	{
		arel.Debug.log(config.vids[targetVidId].path);
		
		arel.Scene.setFreezeTracking(true);
		arel.Scene.stopCamera();
		
		//arel.Media.startVideo(config.vids[targetVidId].path);
		setTimeout(startVideo, 2000, config.vids[targetVidId].path);
	}
}
...
function callFlashToDownloadVid($targetId)
{
	...
	document.getElementById("download-box").innerHTML = "wait...";
